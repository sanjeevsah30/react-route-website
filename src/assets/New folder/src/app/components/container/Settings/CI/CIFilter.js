...
import SubFilterV2 from "../Audit Manager/CallFiltersV3/SubFilterV2";
import { deleteCISubFilter } from "@store/cutsomerIntelligence/internalCiDashboardSlice";
const { TextArea } = Input;

function CIFilter({ handleActiveTab, questionToEdit, editEnable, activeTab }) {
    ...
    const removeSubFilters = (id) => {
        ...
        if (typeof id === "number") {
            dispatch(deleteCISubFilter).then((res) => {
                setSubfilters([...newSubFilters]);
            });
        } else {
            setSubfilters([...newSubFilters]);
